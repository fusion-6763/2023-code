...
public class RobotContainer {
 ...
 // private final Joystick vroomSJoystick = new Joystick(
//  OperatorConstants.kDriverControllerPort);
  /**
  ...
  public Command getAutonomousCommand() {
    ...
    double STRAIGHT = 0;
     ...
     // Commands.run(() -> intake.neutral(), intake),
      ...
      Command sequence2 = new SequentialCommandGroup(
        // START SPIT
      Commands.run(() -> intake.backward(), intake).withTimeout(0.5 * TIME_SCALE), 
      Commands.run(() -> intake.neutral(), intake).withTimeout(0.5 * TIME_SCALE),
      ...
      // GO TO BALL
      Commands.run(() -> drive.customDrive(0, FORWARDS * SPEED), drive).withTimeout(1.38 * TIME_SCALE),
      ...
      // STRAIGHEN
      Commands.run(() -> drive.customDrive(RIGHT * .8, BACKWARDS * TURNSPEED)).withTimeout(0.65 * TIME_SCALE),
      
      // DRIVE TO GOAL
      Commands.run(() -> drive.customDrive(0, FORWARDS * SPEED), drive).withTimeout(1 * TIME_SCALE),
    ...
    Commands.run(() -> drive.customDrive(0, BACKWARDS * 0.8), drive).withTimeout(2)
    );// drive straight 
    //0.85 - 12 feet
    ...
    Commands.run(() -> drive.customDrive(RIGHT * 0.7, FORWARDS * 0.7), drive).withTimeout(1)
    ); //sweep turn
    ...
    return testSequenceStr8;

    // PathPlannerTrajectory examplePath = PathPlanner.loadPath("Basic", new PathConstraints(3, 3));    
